...
// `(e) => {...}` arrow literal to `world.forEachEntity` EVERY fixed step,
// forever - a per-step closure allocation this module's own header claims
// never happens ("nothing here returns or captures a fresh object per
// call"). Same class of bug, same fix, as `World.js`'s `_dispatchEvent`
// (architect review #1, item 5a: "bound once here, not re-created ... on
// every ... call") and BUG-PERF-001 (a)'s `stepBeacon` fix: the per-entity
// callback is now a single module-level function, created once, that reads
// its per-call parameters (world/dt/cfg-derived values, or the actor for
// contacts) off a reused scratch object set immediately before
// `forEachEntity` runs - never rebuilt while entities are being visited.
const _rollerCtx = { world: null, gravity: 0, defaults: null, sleepTime: 0, nFrames: 8 };
let _rollerDt = 0;

function stepOneRoller(e) {
  const { world, gravity, defaults, sleepTime, nFrames } = _rollerCtx;
  ...
  if (!roller.sleeping) {
    ...
    body.vx += gravity * _tilt.x * _rollerDt;
    body.vy += gravity * _tilt.y * _rollerDt;

    const speed = Math.hypot(body.vx, body.vy);
    if (speed > EPS) {
      const newSpeed = Math.max(0, speed - roller.rollFriction * _rollerDt);
  ...
  if (!body.grounded) {
    body.vz -= gravity * _rollerDt;
    t.z += body.vz * _rollerDt;
    if (sector.ceilH !== 'sky') {
...
export function stepRollers(world, dtSec, cfg) {
  // BUG-PERF-001 (b): fill the reused context/dt instead of building a fresh
  // per-call closure (see the `_rollerCtx`/`stepOneRoller` comment above).
  _rollerCtx.world = world;
  _rollerCtx.gravity = (cfg && cfg.gravity) || 0;
  _rollerCtx.defaults = (cfg && cfg.rollerDefaults) || {};
 ...
 */
// BUG-PERF-001 (b): same fix as `_rollerCtx`/`stepOneRoller` above - the
// per-roller contact test is a single module-level function, created once,
// reading its per-call parameters off this reused context.
const _contactCtx = {
  world: null, actor: null, actorBody: null, at: null,
  pushMinSpeed: 0, pushSpeedScale: 1, actorHeight: 0, actorStepUpMax: 0,
  startX: 0, startY: 0,
};

function resolveOneContact(e) {
  const {
    world, actorBody, at, pushMinSpeed, pushSpeedScale, actorHeight, actorStepUpMax, startX, startY,
  } = _contactCtx;
  const roller = e.components && e.components.roller;
  const body = e.components && e.components.body;
  if (!roller || !body) return;
...
export function resolveBodyContacts(world, actor, cfg) {
  ...
  _contactCtx.world = world;
  _contactCtx.actor = actor;
  _contactCtx.actorBody = actorBody;
  _contactCtx.at = at;
  _contactCtx.pushMinSpeed = (cfg && cfg.pushMinSpeed) || 0;
  _contactCtx.pushSpeedScale = typeof (cfg && cfg.pushSpeedScale) === 'number' ? cfg.pushSpeedScale : 1;
  _contactCtx.actorHeight = (cfg && cfg.height) || 0;
  _contactCtx.actorStepUpMax = (cfg && cfg.stepUpMax) || 0;
