...
**2026-09-23, US-029 item 5 parity: `qfloor` in `DP.util.shade`.** The engine (`engine/render/detailShade.js`, GLSL twin in `common.js`) floors block/course boundaries with `qfloor(x) = Math.floor(x + 1/256)` so f64 JS and f32 GPU agree (e.g. v = 2.8 on brick tops). The reference now uses the same `qfloor` at the same four places: `course`, `bix`, band `pos` and `crossLine`'s `k`. Nothing else changed. This restores the `?shadetest=1` v2 table (had dropped to 1774/2242). Other `Math.floor` calls (hash texel coords, `pick`, `level`, tints) stay as they were, matching the engine.
