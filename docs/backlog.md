...
| 6 | US-004b | **Sector caster: overdraw 1.0x, allocation-free ray loop, fast shader, headless bench** (engine story) | P0 | dev | **ARCH CHANGES #2 (architect, 2026-09-24): item 1 (skylight far ceiling) is NOT fixed - start pose column 80 rows 22-25 are still sky; one-line cause in the story section. Items 2-6 OK.** Rework (2026-09-24): all 6 ARCH CHANGES items done - skylight far-ceiling fix, bench pose 3 + sky-cell counts, bench checks (baseline/fast-vs-ref/skyFallback:false), heap-delta check, `fastShade.js` one-liner, notes corrected. Back to architect. Must be `done` before US-006 and US-016 |
