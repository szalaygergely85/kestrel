...
| 6 | US-004b | **Sector caster: overdraw 1.0x, allocation-free ray loop, fast shader, headless bench** (engine story) | P0 | dev | Architect re-review #3 (2026-09-24): ARCH CHANGES - column 80 fixed; the solid-cell branch skips the solid cell's own ceiling segment (sky over a low `w` wall never bounded, and the next ceiling is stretched over the wall), see section. Back to programmer. Must be `done` before US-006 and US-016 |
