...
| 10a | US-028 | **Detail pass v2: G-buffer shading, texel-class glyphs, edge pass, fog v2** (engine story) | P0 | dev | Architect review 1 (2026-09-23): ARCH CHANGES - programmer items 1-5 (MaterialTable flattening, baked mat ids, relief masks, same-surface parity tool + floor root cause, bench), designer (export samples, `ceiling_timber` tuning). Budget stands. Must be `done` before US-006 |
...
### US-028 Detail pass v2: G-buffer shading, texel-class glyphs, edge pass, fog v2  [Priority: P0] [Status: dev]
