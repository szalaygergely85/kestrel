...
| 6 | US-004b | **Sector caster: overdraw 1.0x, allocation-free ray loop, fast shader, headless bench** (engine story) | P0 | arch-review | Programmer (2026-09-24): fixed the solid-cell branch per re-review #3 (single `castFloorCeiling` call for the solid cell's own ceiling/sky segment). Both named probe cells verified, new bench pose added, bench + `?shadetest=1` + physics all pass. Awaiting architect re-review #4. Must be `done` before US-006 and US-016 |
