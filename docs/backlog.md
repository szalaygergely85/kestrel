...
| 6 | US-004b | **Sector caster: overdraw 1.0x, allocation-free ray loop, fast shader, headless bench** (engine story) | P0 | dev | **ARCH CHANGES (architect, 2026-09-23)** - 6 numbered items in the story section (sky-band/skylight fix, bench pose 3, bench checks per AC, heap-delta check, one-line fastShade fix, notes correction). Programmer, then back to architect. Must be `done` before US-006 and US-016 |
...
| 13 | US-009 | Physics: jump, step-up, landing feel | P0 | dev | Programmer |
...
### US-004b Sector caster: overdraw to 1.0x, allocation-free ray loop, fast shader, headless bench  [Priority: P0] [Status: dev]
...
- [x] **Overdraw 1.0x:** cells written per frame == cols x rows (9,600) on all 4 poses, and every cell is written exactly once, with `skyFallback: true` (today's `test_room` mode). With `skyFallback: false`: cells written + open-span cells == 9,600, with no cell written twice. The two sources in architecture.md 12 item 1 are fixed:
  ...
  - **ASK ARCHITECT: two more overdraw/underdraw sources existed beyond these two, found empirically while chasing "exactly 9,600" - see "Overdraw sources beyond the sketch" in the programmer notes below. Fixed, but they mean the pre-fast-shader image is not byte-identical to the baseline (next criterion) - please confirm this is the right call.**
...
- [~] **Image identity:**
  - Before the fast shader is switched on (overdraw, allocation and `OpenSpans` work only), the checksum at all 4 poses is **byte-identical** to the baseline. **Not met as literally written - see the ASK ARCHITECT note above and the programmer notes below: 2 extra overdraw/gap fixes were needed to hit exactly 9,600 writes/pose, and every differing cell was verified (by diffing against a write-count tracker on the baseline code) to be a cell the baseline wrote MORE than once - i.e. an already-ambiguous "last write wins" pixel, not a clean, singly-resolved one. 0 such regressions found on 3 of 4 poses; the 4th ("start") has 10 diffs, all in the same previously-identified gap.**
  - After the fast shader, the glyph checksum is identical to the baseline, and fg/bg are within ±4 per channel of the baseline on every cell. The bench checks this per cell, not only via the checksum. **Met against this story's OWN corrected pre-fast-shader output** (see numbers below): all 4 poses' `glyphIdx` checksums are byte-identical between `--shader=reference` and the default fast shader, and `?shadetest=1` (361 samples: 5 legacy + 12 materials x 8 distances x 4 heights + 5 sky) all pass, worst deviation 0.25 (tolerance ±4).
  - This can be done as two commits, or with a switch that selects the reference shader in the bench. **Done as a switch: `castScene(..., { shader: 'reference' | 'fast' })`, default `'fast'`.**
...
- For the tester: run `node tools/bench-cast.mjs` and `node tools/bench-cast.mjs --gc` (exit 0, numbers match the notes within noise). Run `?shadetest=1` (all pass). Walk `test_room` and compare it by eye with the US-004 look at the same poses; there must be no visible change. **Per the ASK ARCHITECT note above, a handful of cells (mostly a 2-row gap over a low wall/pillar, and a couple of stray boundary pixels) will actually look slightly DIFFERENT/more-correct than the US-004 tester's approved screenshots - please have the tester specifically eyeball those spots (see programmer notes) rather than treating any difference as a bug.**
...
### US-009 Physics: jump, step-up, landing feel  [Priority: P0] [Status: dev]
