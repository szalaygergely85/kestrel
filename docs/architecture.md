...
**4. Mounts.** `ModelDef.voxel.mounts?: { [name]: { at: [x, y, z] /* voxel units, rest frame */, part?: string } }`, validated (name pattern, `at` finite inside `size`, `part` exists). `voxelMountWorld(pm, inst, mountIndex, out: Float64Array(3))` = `W * M_part * at` (follows the part's pose). Consumers: US-042 talk prompt anchor, US-022 relay glow. In M1 the flame/glow billboards stay **separate prop entities** in level data (their own `props` entries at the rim, level coordinates) and point lights stay `def.lights` (7.5 item 5): no attach system now.
